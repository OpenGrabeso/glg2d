...
abstract class ShapePipeline extends AbstractShaderPipeline {
    private int vertCoordBuffer = -1;
    private int vertCoordLocation = -1;
    private int vertexArrayId = -1;

    ShapePipeline(String directory, String vertexShaderFileName, String fragmentShaderFileName) {
        super(directory, vertexShaderFileName, null, fragmentShaderFileName);
    }

    private void bindBuffer(GL2GL3 gl) {
        if (vertexArrayId < 0) {
            int[] vao = new int[]{0};
    ...
    void draw(GL2GL3 gl, int mode, FloatBuffer vertexBuffer) {
        bindBuffer(gl);
